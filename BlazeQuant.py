...
#         st.write(f"Volatility of {cryptos[i]}",float(volatility.value))
    ...
    # fig.show()
    st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
    st.write("Above is a plot of volatilities of INJ and related assets. The recent price surge in INJ has surely impacted it's volatility in the present times")
...
def srb():
    ...
    def get_stock_price(symbol):
      df = yf.download('INJ-USD', start='2022-02-01', threads= False)
